...
import { expectedResults, forecastMockVacationTypes, mockNonProjectTimes } from "./__mocks__/forecastMocks";
beforeEach(() => {
    jest.clearAllMocks();
});
describe("Expected time left test, including different non project time types", () => {
    ...
    describe.each(expectedResults)("Check vacation calculations", (fakePersonId, expectedResult) => {
        it(`Should return proper exact time for person ID ${fakePersonId}`, () => {
            const result = TimeUtilities.checkIfVacationCaseExists(fakePersonId, forecastMockVacationTypes, mockNonProjectTimes, fakeStartDate, fakeEndDate);
            expect(result).toBe(expectedResult);
        });
    });
});
//# sourceMappingURL=vacationCalculationTest.test.js.map