...
import type { File, PdfFile } from "../schema/google";

const folderId = config.get().google.driveFolderId;
...
export const getPersonalGoogleAuth = async () => {
  const { clientId, clientSecret, refreshToken } = config.get().google;
...
export const getDriveService = async () => {
  // TODO: need to be changed back to getGoogleAuth when service account is set up
  const auth = await getPersonalGoogleAuth();
 ...
 * @returns array of File objects
 */
export const getFilesInYear = async (
  year: string,
